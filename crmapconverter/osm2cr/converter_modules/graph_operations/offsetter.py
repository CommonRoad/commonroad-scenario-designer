...
"""
from converter_modules.graph_operations import lane_linker, road_graph as rg
    ...
    for node in graph.nodes:
                ...
                if edge.node1 == node and edge.oneway:
                    ...
                    if len(incoming) < len(outgoing):
                        ...
                        if lane_linker.merge_left(incoming, outgoing):
                            offset *= -1
                        edge.interpolated_waypoints = geometry.offset_polyline(edge.interpolated_waypoints, offset,
                                                                               True)
                        ...
                        if lane_linker.merge_left(incoming, outgoing):
                            offset *= -1
                        other.interpolated_waypoints = geometry.offset_polyline(other.interpolated_waypoints, offset,
                                                                                False)
        if node.get_degree() == 3:
            ...
            for edge_index, edge in enumerate(edges):
                ...
                if edge.node2 == node and edge.oneway and edge.forward_successor is not None:
