...
from converter_modules.graph_operations.restrictions import Restriction
from converter_modules.osm_operations import info_deduction as i_d
from converter_modules.utility import idgenerator
from converter_modules.utility.custom_types import Road_info
from converter_modules.utility.geometry import Point, is_corner, Area, lon_lat_to_cartesian


def read_custom_bounds(root) -> Optional[Tuple[float, float, float, float]]:
    bounds = None
    for bound in root.findall('custom_bounds'):
        bounds = (bound.attrib['lat2'], bound.attrib['lon1'], bound.attrib['lat1'], bound.attrib['lon2'])
...
def parse_restrictions(restrictions: Set[ElTree.Element]) -> Dict[int, Set[Restriction]]:
    """
    ...
    for restriction_element in restrictions:
        from_edge_id, to_edge_id, via_element_id, via_element_type = None, None, None, None
...
def parse_file(filename: str, accepted_highways: List[str]) \
        -> Tuple[
            Set[ElTree.Element], Dict[int, Point], type(None), Tuple[float, float], Tuple[float, float, float, float]]:
    """
...
def extract_tag_info(road: ElTree.Element) -> Tuple[Road_info, int]:
    ...
    return (nr_of_lanes, forward_lanes, backward_lanes, oneway, turnlanes, turnlanes_forward,
            turnlanes_backward), speedlimit


def get_graph_nodes(roads: Set[ElTree.Element], points: Dict[int, Point]) -> Dict[int, rg.GraphNode]:
    """
...
def get_area_from_bounds(bounds: Tuple[float, float, float, float], origin: np.ndarray) -> Area:
...
def get_graph_edges_from_road(roads: Set[ElTree.Element],
                              ...
                              origin: np.ndarray) -> Dict[int, Set[rg.GraphEdge]]:
    """
    ...
    for road_index, road in enumerate(roads):
            ...
            if config.SPLIT_AT_CORNER and is_corner(index, waypoints):
                try:
                    ...
                    if waypoint_id not in nodes:
                        ...
                        nodes[waypoint_id] = rg.GraphNode(id_int, waypoint.x, waypoint.y, set())
                    edge_nodes.append((nodes[waypoint_id], index))
                except ValueError:
        ...
        for index, node in enumerate(edge_nodes[:-1]):
            ...
            if len(current_waypoints) >= 2:
                ...
                new_edge = rg.GraphEdge(edge_id, node1, node2, current_waypoints, lane_info, assumptions, speedlimit,
                                        roadtype)
            ...
            if len(from_edges) == 1:
                ...
                for restriction in restrictions:
                    ...
                    if restriction.via_element_type == "node":
                        if restriction.via_element_id in nodes \
                                and nodes[restriction.via_element_id] in (from_edge.node1, from_edge.node2):
                ...
                print("several edges have the same id, we cannot apply restrictions to it")
                # TODO implement restrictions for mutliple edges with same id
                pass
        else:
            print("unknown id '{}' for restriction element. skipping restriction".format(from_id))
...
def roads_to_graph(roads: Set[ElTree.Element], road_points: Dict[int, Point],
                   restrictions: Dict[int, Set[Restriction]], center_point: Tuple[float, float],
                   bounds: Tuple[float, float, float, float], origin: tuple) -> rg.Graph:
    """
