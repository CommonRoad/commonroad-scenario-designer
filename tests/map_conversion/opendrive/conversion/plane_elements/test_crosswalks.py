...
class MyTestCase(unittest.TestCase):
    def test_crosswalk_conversion(self):
        ...
        outline = [
            [4.5497031009155489, 1.4666291880958617],
    ...
    def test_four_way_crossing(self):
        ...
        np.testing.assert_array_almost_equal(
            lanelet_7.left_vertices, [[4.30983202, -4.85975667], [4.34835011, 3.56917188]]
        )
        np.testing.assert_array_almost_equal(
            lanelet_7.right_vertices, [[6.80830036, -5.20064224], [6.84993415, 3.91009668]]
        )
        np.testing.assert_array_almost_equal(
            lanelet_7.center_vertices, [[5.55906619, -5.03019945], [5.59914213, 3.73963428]]
        )

        np.testing.assert_array_almost_equal(
            lanelet_6.left_vertices, [[-4.11917897, -4.86917131], [4.30983202, -4.85975667]]
        )
        np.testing.assert_array_almost_equal(
            lanelet_6.right_vertices, [[-4.46997289, -7.36956469], [4.64072751, -7.35938864]]
        )
        np.testing.assert_array_almost_equal(
            lanelet_6.center_vertices, [[-4.29457593, -6.119368], [4.47527977, -6.10957265]]
        )

        np.testing.assert_array_almost_equal(
            lanelet_11.left_vertices, [[-4.08066122, 3.55975695], [-4.11917897, -4.86917131]]
        )
        np.testing.assert_array_almost_equal(
            lanelet_11.right_vertices, [[-6.57912958, 3.90064261], [-6.620763, -5.21009635]]
        )
        np.testing.assert_array_almost_equal(
            lanelet_11.center_vertices, [[-5.3298954, 3.73019978], [-5.36997099, -5.03963383]]
        )

        np.testing.assert_array_almost_equal(
            lanelet_12.left_vertices, [[4.34835011, 3.56917188], [-4.08066122, 3.55975695]]
        )
        np.testing.assert_array_almost_equal(
            lanelet_12.right_vertices, [[4.6991442, 6.06956526], [-4.4115563, 6.05938891]]
        )
        np.testing.assert_array_almost_equal(
        ...
        self.assertSetEqual(
            scenario.lanelet_network.find_intersection_by_id(78).crossings, {6, 7, 11, 12}
        )
