...
from crdesigner.conversion.lanelet_lanelet2.lanelet2 import OSMLanelet, WayRelation, DEFAULT_PROJ_STRING, Node, \
    RightOfWayRelation
...
def _add_stop_line_to_lanelet(lanelets: List[Lanelet], stop_lines: List[StopLine]):
    ...
    for l in lanelets:
        for s in stop_lines:
            if l.polygon.shapely_object.intersects(LineString([s.start, s.end])):
                l.stop_line = s
