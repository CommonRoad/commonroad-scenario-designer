...
from crdesigner.verification_repairing.map_verification_repairing import \
    verify_and_repair_scenario, verify_and_repair_dir_maps

cli = typer.Typer(help="Toolbox for Map Conversion and Scenario Creation for Autonomous Vehicles")


def store_scenario(sc: Scenario, output_file: str, force_overwrite: bool, author: str, affiliation: str,
...
def main(ctx: typer.Context,
         ...
         author: Annotated[str, typer.Option(..., help="Your name")] = "", affiliation: Annotated[
...
def verify_map(ctx: typer.Context):
    ...
    if not valid:
        ...
        if not ctx.obj["force_overwrite"]:
            file_path = file_path.replace('.xml', '') + '-repaired.xml' if file_path[-4:] == ".xml" \
...
def odrcr(ctx: typer.Context):
    ...
    store_scenario(scenario, ctx.obj["output_file"], ctx.obj["force_overwrite"], ctx.obj["author"],
                   ctx.obj["affiliation"], ctx.obj["tags"])


@cli.command()
def lanelet2cr(ctx: typer.Context,
               proj: Annotated[
                   Optional[str], typer.Option(..., help="Overwrite existing CommonRoad file")] = None,
               adjacencies: Annotated[
                   bool, typer.Option(..., help="Detect left and right adjacencies of "
                                                "lanelets if they do not share a common way")] = True,
               left_driving: Annotated[
                   bool, typer.Option(..., help="set to true if map describes a left driving "
...
@cli.command()
def crlanelet2(ctx: typer.Context,
               proj: Annotated[
                   Optional[str], typer.Option(..., help="Overwrite existing CommonRoad file")] = None,
