...
from crdesigner.ui.gui.model.config_default_model import config_default_model as config_default
from crdesigner.ui.gui.utilities import transfer
from crdesigner.ui.gui.view.settings.osm_settings_ui import OSMSettingsUI
from crdesigner.ui.gui.view.settings.osm_sub_settings_ui import EditLaneCounts, EditStreetTypes, EditLaneWidth, \
    EditSpeedLimits, EditSublayerWayTypes
from crdesigner.ui.gui.view.settings.settings_ui import HEIGHT, COLUMNS, WIDTHF, WIDTHM, FACTOR


class OSMSettingsController:
    ...
    def __init__(self, main_app, close_action):
        ...
        self.osm_settings_ui = OSMSettingsUI(h=HEIGHT, c=COLUMNS, wf=WIDTHF, wm=WIDTHM, f=FACTOR)
        self.osm_settings_ui.setupUi(self.parent.settings_ui.tabWidget)
        self.parent.settings_ui.tabWidget.addTab(self.osm_settings_ui.scrollArea, "OSM")
    ...
    def connect_events(self, window) -> None:
        """
        ...
        window.btn_edit_sublayer_way_types.clicked.connect(
                self.edit_sublayer_way_types)  # window.btn_save.clicked.connect(self.save_button)
    ...
    def update_ui_values(self) -> None:
        ...
        window.sb_intersection_distance_sublayer.setValue(config.INTERSECTION_DISTANCE_SUBLAYER)
    ...
    def save_button(self) -> None:
        ...
        :return: None
        """
        if not coordinates_from_text(self.osm_settings_ui.le_coordinates.text())[0]:
            QMessageBox.warning(self, "Warning", "The entered coordinates are invalid! Settings could not be saved.",
                                QMessageBox.Ok, )
            return
        self.save_to_config()
        write_config()
...
def write_config():
    ...
    file_path, _ = QFileDialog.getSaveFileName(None, "Store Settings", "", "python file *.py (*.py)",
