        ...
        if self.current_scenario.get_current_scenario() is not None:
            ...
            if self.current_scenario.get_current_scenario().lanelet_network.location:
                self.scenario_toolbox_ui.scenario_geo_name_id.setText(
                    str(
                        self.current_scenario.get_current_scenario().lanelet_network.location.geo_name_id
                    )
                )
                self.scenario_toolbox_ui.scenario_latitude.setText(
                    str(
                        self.current_scenario.get_current_scenario().lanelet_network.location.gps_latitude
                    )
                )
                self.scenario_toolbox_ui.scenario_longitude.setText(
                    str(
                        self.current_scenario.get_current_scenario().lanelet_network.location.gps_longitude
                    )
                )
                    ...
                    if self.current_scenario.get_current_scenario().lanelet_network.location.geo_transformation:
                        location = Location(
                            ...
                            geo_transformation=GeoTransformation(
                                geo_reference=self.current_scenario.get_current_scenario().lanelet_network.location.geo_transformation.geo_reference,
                                x_translation=self.current_scenario.get_current_scenario().lanelet_network.location.geo_transformation.x_translation,
                                y_translation=self.current_scenario.get_current_scenario().lanelet_network.location.geo_transformation.y_translation,
                            ),
                        )
                    else:
                        location = Location(
