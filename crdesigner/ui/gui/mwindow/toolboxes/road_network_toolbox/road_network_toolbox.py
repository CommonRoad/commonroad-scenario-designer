...
from crdesigner.ui.gui.mwindow.toolboxes.road_network_toolbox.road_network_toolbox_ui import (
...
class RoadNetworkToolbox(
    ...
    def connect_gui_elements(self):
        ...
        self.initialized = False
        self.road_network_toolbox_ui.button_add_lanelet.clicked.connect(
            lambda: self.add_lanelet()
        )
        ...
        self.road_network_toolbox_ui.button_create_traffic_lights.clicked.connect(
            lambda: self.create_traffic_lights()
        )
    ...
    def collect_lanelet_ids(self) -> List[int]:
        ...
        if self.current_scenario is not None:
            return sorted(
                [la.lanelet_id for la in self.current_scenario.lanelet_network.lanelets]
            )
        else:
            return []
    ...
    def collect_traffic_sign_ids(self) -> List[int]:
        ...
        if self.current_scenario is not None:
            return sorted(
                [
                    ts.traffic_sign_id
                    for ts in self.current_scenario.lanelet_network.traffic_signs
                ]
            )
        return []
    ...
    def collect_traffic_light_ids(self) -> List[int]:
        ...
        if self.current_scenario is not None:
            return sorted(
                [
                    tl.traffic_light_id
                    for tl in self.current_scenario.lanelet_network.traffic_lights
                ]
            )
        return []
        ...
        if self.current_scenario is not None:
            return sorted(
                [
                    ...
                    for inter in self.current_scenario.lanelet_network.intersections
                ]
            )
        return []
    ...
    def collect_incoming_lanelet_ids_from_intersection(self) -> List[int]:
        ...
        if self.road_network_toolbox_ui.selected_intersection.currentText() not in [
            ...
            intersection = (
                self.current_scenario.lanelet_network.find_intersection_by_id(
    ...
    def initialize_basic_lanelet_information(self):
        ...
        if self.road_network_toolbox_ui.connect_to_previous_selection.isChecked():
            if self.last_added_lanelet_id is not None:
                last_lanelet = self.current_scenario.lanelet_network.find_lanelet_by_id(
                    self.last_added_lanelet_id
                )
                self.road_network_toolbox_ui.previous_lanelet.addItem(
                    str(last_lanelet.lanelet_id)
                )

                self.road_network_toolbox_ui.lanelet_length.setText(
                    self.get_length(last_lanelet)
                )
                self.road_network_toolbox_ui.lanelet_width.setText(
                    self.get_width(last_lanelet)
                )
                ...
                for i in range(0, len(line_markings)):
                    if (
                        line_markings[i]
                        == last_lanelet.line_marking_right_vertices.value
                    ):
                        break
                ...
                for i in range(0, len(line_markings)):
                    if (
                        line_markings[i]
                        == last_lanelet.line_marking_left_vertices.value
                    ):
                        break
                ...
                for i in range(0, len(road_user_list) - 1):
                    self.road_network_toolbox_ui.road_user_bidirectional.addItem(
                        road_user_list[i]
                    )
                    item = self.road_network_toolbox_ui.road_user_bidirectional.model().item(
                        i, 0
                    )
                ...
                for i in range(0, len(road_user_list) - 1):
                    self.road_network_toolbox_ui.road_user_oneway.addItem(
                        road_user_list[i]
                    )
                    item = self.road_network_toolbox_ui.road_user_oneway.model().item(
                        i, 0
                    )
                ...
                for i in range(0, len(lanelet_type_list) - 1):
                    self.road_network_toolbox_ui.lanelet_type.addItem(
                        lanelet_type_list[i]
                    )
            ...
            for i in range(0, len(road_user_list) - 1):
                ...
                item = (
                    self.road_network_toolbox_ui.road_user_bidirectional.model().item(
                        i, 0
                    )
                )
                item.setCheckState(Qt.Unchecked)
        ...
        self.road_network_toolbox_ui.line_marking_stop_line.addItems(
            line_markings_stop_line
        )
        ...
        self.road_network_toolbox_ui.lanelet_referenced_traffic_sign_ids.setCurrentIndex(
            0
        )
        ...
        self.road_network_toolbox_ui.lanelet_referenced_traffic_light_ids.setCurrentIndex(
            0
        )
    ...
    def initialize_predecessor_and_successor_fields(self):
        ...
        if self.road_network_toolbox_ui.connect_to_predecessors_selection.isChecked():
            ...
            if not pred.isdigit():
                return
            lanelet = self.current_scenario.lanelet_network.find_lanelet_by_id(
                int(pred)
            )
        elif self.road_network_toolbox_ui.connect_to_successors_selection.isChecked():
        ...
        self.road_network_toolbox_ui.number_vertices.setText(
    ...
    def set_default_road_network_list_information(self):
        ...
        self.road_network_toolbox_ui.selected_lanelet_referenced_traffic_sign_ids.setCurrentIndex(
            0
        )
        ...
        self.road_network_toolbox_ui.referenced_lanelets_traffic_light.setCurrentIndex(
            0
        )
        ...
        self.road_network_toolbox_ui.intersection_lanelet_to_fit.addItems(
            ...
            + [
                str(item)
                for item in self.collect_incoming_lanelet_ids_from_intersection()
            ]
        )
    ...
    def add_lanelet(
        ...
        predecessors = [
            int(pre)
            for pre in self.road_network_toolbox_ui.predecessors.get_checked_items()
        ]
        successors = [
            int(suc)
            for suc in self.road_network_toolbox_ui.successors.get_checked_items()
        ]
        ...
        lanelet_angle = np.deg2rad(
            self.get_float(self.road_network_toolbox_ui.lanelet_angle)
        )
        ...
        if connect_to_last_selection:
            ...
            if len(predecessors) > 0:
                predecessor = self.current_scenario.lanelet_network.find_lanelet_by_id(
                    predecessors[0]
                )
                lanelet.translate_rotate(
                    np.array(
                        [
                            predecessor.center_vertices[-1][0],
                            predecessor.center_vertices[-1][1],
                        ]
                    ),
                    0,
                )
            ...
            if len(successors) > 0:
                successor = self.current_scenario.lanelet_network.find_lanelet_by_id(
                    successors[0]
                )
            ...
            lanelet.translate_rotate(
                np.array([lanelet_start_pos_x, lanelet_start_pos_y]), 0
            )
    ...
    def selected_lanelet(self) -> Union[Lanelet, None]:
        ...
        if self.road_network_toolbox_ui.selected_lanelet_update.currentText() not in [
            ...
            self.road_network_toolbox_ui.selected_lanelet_update.currentText()
            in ["None", ""]
            and not self.update
        ):
    ...
    def update_lanelet(self):
        ...
        self.add_updated_lanelet(
            selected_lanelet.lanelet_id,
            selected_lanelet.left_vertices,
            selected_lanelet.right_vertices,
        )

        for la_id in successors:
            self.current_scenario.lanelet_network.find_lanelet_by_id(
                la_id
            ).add_successor(selected_lanelet_id)
        for la_id in predecessors:
            self.current_scenario.lanelet_network.find_lanelet_by_id(
                la_id
            ).add_predecessor(selected_lanelet_id)
        for la_info in adjacent_left:
            self.current_scenario.lanelet_network.find_lanelet_by_id(
                la_info[0]
            ).adj_left = selected_lanelet_id
            self.current_scenario.lanelet_network.find_lanelet_by_id(
                la_info[0]
    ...
    def add_updated_lanelet(
        ...
        successors = [
            int(suc)
            for suc in self.road_network_toolbox_ui.selected_successors.get_checked_items()
        ]
        ...
        adjacent_left = (
            ...
            if self.road_network_toolbox_ui.selected_adjacent_left.currentText()
            != "None"
            else None
        )
        adjacent_right = (
            int(self.road_network_toolbox_ui.selected_adjacent_right.currentText())
            if self.road_network_toolbox_ui.selected_adjacent_right.currentText()
            != "None"
            else None
        )
        adjacent_left_same_direction = (
        ...
        lanelet_length = self.get_float(
            self.road_network_toolbox_ui.selected_lanelet_length
        )
        lanelet_radius = self.get_float(
            self.road_network_toolbox_ui.selected_lanelet_radius
        )
        lanelet_angle = np.deg2rad(
            self.get_float(self.road_network_toolbox_ui.selected_lanelet_angle)
        )
        add_curved_selection = (
        ...
        if add_curved_selection:
            ...
            rotation_angle = math.degrees(
                math.asin((lanelet_end_pos_y - lanelet_start_pos_y) / lanelet_length)
            )
        ...
        if (
            ...
            and not stop_line_at_end
            and not stop_line_at_beginning
        ):
    ...
    def update_lanelet_information(self, lanelet: Lanelet = None):
        ...
        self.road_network_toolbox_ui.selected_lanelet_start_position_x.setText(
            str(
                ...
                else lanelet.center_vertices[0][0]
            )
        )
        self.road_network_toolbox_ui.selected_lanelet_start_position_y.setText(
            str(
                ...
                else lanelet.center_vertices[0][1]
            )
        )
        self.road_network_toolbox_ui.selected_lanelet_end_position_x.setText(
            str(
                0.0
                if lanelet.center_vertices[len(lanelet.center_vertices) - 1][0]
                == 1.0e-16
                else lanelet.center_vertices[len(lanelet.center_vertices) - 1][0]
            )
        )
        self.road_network_toolbox_ui.selected_lanelet_end_position_y.setText(
            str(
                0.0
                if lanelet.center_vertices[len(lanelet.center_vertices) - 1][1]
                == 1.0e-16
                else lanelet.center_vertices[len(lanelet.center_vertices) - 1][1]
            )
        )
        self.road_network_toolbox_ui.selected_lanelet_width.setText(
            self.get_width(lanelet)
        )
        self.road_network_toolbox_ui.selected_lanelet_length.setText(
            self.get_length(lanelet)
        )
        ...
        if not self.lanelet_is_straight(lanelet):
            ...
            self.road_network_toolbox_ui.selected_curved_checkbox.box.setMaximumSize(
                0, 0
            )
            self.road_network_toolbox_ui.selected_curved_checkbox.button.setDisabled(
                True
            )
        else:
            self.road_network_toolbox_ui.selected_curved_checkbox.button.setEnabled(
                True
            )
            self.road_network_toolbox_ui.selected_curved_checkbox.setChecked(False)
            self.road_network_toolbox_ui.selected_curved_checkbox.box.setMaximumSize(
        ...
        self.road_network_toolbox_ui.selected_adjacent_left.setCurrentText(
            str(lanelet.adj_left)
        )
        self.road_network_toolbox_ui.selected_adjacent_right.setCurrentText(
            str(lanelet.adj_right)
        )
        ...
        if lanelet.stop_line is not None:
            ...
            if all(lanelet.stop_line.start == lanelet.left_vertices[0]) and all(
                ...
                self.road_network_toolbox_ui.selected_stop_line_beginning.setChecked(
                    True
                )
                self.road_network_toolbox_ui.adjust_selected_stop_line_position()
            elif all(
                lanelet.stop_line.start
                == lanelet.left_vertices[len(lanelet.left_vertices) - 1]
            ) and all(
                lanelet.stop_line.end
                == lanelet.right_vertices[len(lanelet.right_vertices) - 1]
            ):
                self.road_network_toolbox_ui.selected_stop_line_end.setChecked(True)
                self.road_network_toolbox_ui.adjust_selected_stop_line_position()
            else:
                self.road_network_toolbox_ui.selected_stop_line_select_position.setChecked(
                    True
                )
                self.road_network_toolbox_ui.adjust_selected_stop_line_position()
    ...
    def create_adjacent(self):
        ...
        adjacent_left = (
            self.road_network_toolbox_ui.create_adjacent_left_selection.isChecked()
        )
        ...
        if selected_lanelet.stop_line is not None:
            ...
            if all(
                ...
                == selected_lanelet.left_vertices[
                    len(selected_lanelet.left_vertices) - 1
                ]
            ) and all(
    ...
    def remove_lanelet(self):
        ...
        MapCreator.remove_lanelet(
            selected_lanelet.lanelet_id, self.current_scenario.lanelet_network
        )
    ...
    def add_four_way_intersection(self):
        ...
        incoming_length = int(
            self.road_network_toolbox_ui.intersection_incoming_length.text()
        )
        add_traffic_signs = (
            self.road_network_toolbox_ui.intersection_with_traffic_signs.isChecked()
        )
        add_traffic_lights = (
        ...
        country_signs = globals()[
            ...
            + SupportedTrafficSignCountry(
                self.current_scenario.scenario_id.country_id
            ).name.capitalize()
        ]
    ...
    def add_traffic_sign(self, traffic_sign_id: int = None):
        ...
        # Check if 'None' is and other items are selected -> Uncheck 'None'
            ...
            self.road_network_toolbox_ui.referenced_lanelets_traffic_sign.uncheck_items(
                "None"
            )
        country_signs = globals()[
            "TrafficSignID"
        ...
        for row in range(
            ...
            if (
                self.road_network_toolbox_ui.traffic_sign_element_table.item(row, 1)
                is None
            ):
                additional_value = []
            else:
                additional_value = [
                    self.road_network_toolbox_ui.traffic_sign_element_table.item(
                        row, 1
                    ).text()
                ]
        ...
        traffic_sign_id = (
            ...
            else self.current_scenario.generate_object_id()
        )
    ...
    def remove_traffic_sign(self):
        ...
        if self.current_scenario is None:
            self.text_browser.append("_Warning:_ Create a new file")
            return
        if self.road_network_toolbox_ui.selected_traffic_sign.currentText() not in [
            "",
            "None",
        ]:
            ...
            return
        traffic_sign = self.current_scenario.lanelet_network.find_traffic_sign_by_id(
    ...
    def update_traffic_sign_information(self):
        ...
        if self.road_network_toolbox_ui.selected_traffic_sign.currentText() not in [
            ...
            traffic_sign = (
                self.current_scenario.lanelet_network.find_traffic_sign_by_id(
                    selected_traffic_sign_id
                )
            )
            referenced_lanelets = [
    ...
    def add_traffic_light(self, traffic_light_id: int = None):
        ...
        # Check if 'None' is and other items are selected -> Uncheck 'None'
            ...
            self.road_network_toolbox_ui.referenced_lanelets_traffic_light.uncheck_items(
                "None"
            )
        referenced_lanelets = {
            int(la)
        ...
        traffic_light_active = (
            self.road_network_toolbox_ui.traffic_light_active.isChecked()
        )
            ...
            if elem == "r" and time_red > 0:
                ...
                traffic_light_cycle.append(
                    TrafficLightCycleElement(
                        TrafficLightState.RED_YELLOW, time_red_yellow
                    )
                )
    ...
    def remove_traffic_light(self):
        ...
        if self.current_scenario is None:
            self.text_browser.append("_Warning:_ Create a new file")
            return
        if self.road_network_toolbox_ui.selected_traffic_light.currentText() not in [
            "",
            "None",
        ]:
            ...
            return
        traffic_light = self.current_scenario.lanelet_network.find_traffic_light_by_id(
            selected_traffic_light_id
        )
    ...
    def update_traffic_light_information(self):
        ...
        if self.road_network_toolbox_ui.selected_traffic_light.currentText() not in [
            ...
            traffic_light = (
                self.current_scenario.lanelet_network.find_traffic_light_by_id(
                    selected_traffic_light_id
                )
            )
            ...
            for elem in traffic_light.cycle:
                ...
                if elem.state is TrafficLightState.RED_YELLOW:
                    cycle_order += "ry-"
                    self.road_network_toolbox_ui.time_red_yellow.setText(
                        str(elem.duration)
                    )
                if elem.state is TrafficLightState.INACTIVE:
                    cycle_order += "in-"
                    self.road_network_toolbox_ui.time_inactive.setText(
                        str(elem.duration)
                    )
            cycle_order = cycle_order[:-1]
            self.road_network_toolbox_ui.traffic_light_cycle_order.setCurrentText(
                cycle_order
            )
    ...
    def create_traffic_lights(self):
        ...
        converter = CR2SumoMapConverter(
            self.current_scenario, SumoConfig.from_scenario(self.current_scenario)
        )
    ...
    def add_incoming_to_table(
        self, new_incoming: bool = True, incoming_ids: List[str] = None
    ):
        """
    ...
    def update_left_of_combobox(self, incoming_ids: List[str] = None):
        ...
        if incoming_ids is None:
            incoming_ids = [
                self.road_network_toolbox_ui.intersection_incomings_table.item(
                    row, 0
                ).text()
                for row in range(
    ...
    def add_intersection(self, intersection_id: int = None):
        ...
        for row in range(
            ...
            if len(incoming_lanelets) < 1:
                ...
                print(
                    "road_network_toolbox.py/add_intersection: An incoming must consist at least of one lanelet."
                )
                return
            successor_left = {
                int(item)
        ...
        if len(incomings) > 1:
            ...
            print(
                "road_network_toolbox.py/add_intersection: An intersection must consist at least of two incomings."
            )
    ...
    def remove_incoming(self):
        ...
        """
        num_rows = self.road_network_toolbox_ui.intersection_incomings_table.rowCount()
        self.road_network_toolbox_ui.intersection_incomings_table.removeRow(
            num_rows - 1
        )
    ...
    def rotate_lanelet(self):
        ...
        selected_lanelet_one.translate_rotate(
            np.array([0, 0]), np.deg2rad(rotation_angle)
        )
        selected_lanelet_one.translate_rotate(
            initial_vertex - selected_lanelet_one.center_vertices[0], 0.0
        )
        self.callback(self.current_scenario)
    ...
    def translate_lanelet(self):
        ...
        selected_lanelet_one.translate_rotate(
            np.array([x_translation, y_translation]), 0
        )
        self.callback(self.current_scenario)
    ...
    def merge_with_successor(self):
        ...
        for suc in selected_lanelet_one.successor:
            new_lanelet = Lanelet.merge_lanelets(
                selected_lanelet_one,
                self.current_scenario.lanelet_network.find_lanelet_by_id(suc),
            )
            self.current_scenario.remove_lanelet(
                self.current_scenario.lanelet_network.find_lanelet_by_id(suc)
            )
    ...
    def fit_intersection(self):
        ...
        if (
            self.road_network_toolbox_ui.selected_intersection.currentText()
            not in ["", "None"]
            and self.road_network_toolbox_ui.other_lanelet_to_fit.currentText()
            not in ["", "None"]
            ...
            predecessor_id = int(
                self.road_network_toolbox_ui.other_lanelet_to_fit.currentText()
            )
            lanelet_predecessor = (
                self.current_scenario.lanelet_network.find_lanelet_by_id(predecessor_id)
            )
            ...
            lanelet_successor = (
                self.current_scenario.lanelet_network.find_lanelet_by_id(successor_id)
            )
...
# TODO preceding lanelet shows all lanelets: other_lanelet_to_fit

# TODO remove invoming doesn't delte lanelets
