...
from crdesigner.crmapver.partitioning.map_partition import LaneletPartitioning, TrafficSignPartitioning, \
...
from crdesigner.crmapver.verification.satisfaction import InvalidStates
from crdesigner.crmapver.verification.formula_ids import LaneletFormulaID, TrafficSignFormulaID, TrafficLightFormulaID, \
    FormulaID, extract_formula_ids_by_type, FormulaTypes, filter_formula_ids_by_type
...
class MapVerifier:
    ...
    def _unpartitioned_verify(self) -> InvalidStates:
        ...
        valid_checker = self._create_verifier(mapping, self._config.verification.formulas, partitioning=False)
    ...
    def _partitioned_verify(self) -> InvalidStates:
        ...
        for formula_type in FormulaTypes:
            ...
            for mapping in mappings:
                ...
                valid_checker = MapVerifier._create_verifier(mapping, formula_ids, partitioning=True)
    ...
    @staticmethod
    def _create_verifier(mapping: HOLMapping, formula_ids: List[FormulaID],
                         partitioning: bool) -> HOLVerificationChecker:
        """
        Creates for the solver a corresponding validity checker.

        :param mapping: Mapping.
        :param formula_ids: Formula IDs.
        :param partitioning: Boolean indicates whether partitioning is used.
