...
class Node:
    ...
    # users must make sure that all points in their osm maps contain elevation tags.
    def __init__(self, id_, lat, lon, ele=0.0, mgrs_code=None, local_x=None, local_y=None):
        self.id_ = str(id_)
        self.lat = str(lat)
        self.lon = str(lon)
        self.mgrs_code = "" # TODO: clarify
        self.local_x = str(local_x)
        self.local_y = str(local_y)
        self.ele = str(ele)
    ...
    def serialize_to_xml(self) -> etree:
        ...
        if self.mgrs_code != "":
            mgrs_code = etree.SubElement(node, "tag") # init node tags
            mgrs_code.set("k", "mgrs_code")
            mgrs_code.set("v", self.mgrs_code)
            node.append(mgrs_code)
        # somehow self.local_x is not None does not work
        if str(self.local_x) != str(None) and str(self.local_y) != str(None):
...
# modify the relation so that it looks similar to:
#   <relation id="-1466">
#     <member type="way" role="left" ref="-1441"/>
#     <member type="way" role="right" ref="-1465"/>
