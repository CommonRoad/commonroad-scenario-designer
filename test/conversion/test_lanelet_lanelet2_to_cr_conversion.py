...
def load_and_convert_opendrive(xodr_file_name: str) -> Scenario:
    ...
    with open(
            ...
            + f"/lanelet_lanelet2_test_files/{xodr_file_name}.osm",
            "r",
    ) as fh:
...
def compare_maps(file_name: str) -> bool:
    ...
    with open(
            os.path.dirname(os.path.realpath(__file__))
            + f"/lanelet_lanelet2_test_files/{xml_output_name}.xml",
            "r",
    ) as fh:
...
class TestLanelet2ToCommonRoad(unittest.TestCase):
    ...
    def test_map_with_priorities(self):
        ...
        self.assertTrue(compare_maps("traffic_priority_lanelets_utm"))

    def test_map_with_speed_limits(self):
        """Basic test file including speed limits."""
        self.assertTrue(compare_maps("traffic_speed_limit_utm"))
